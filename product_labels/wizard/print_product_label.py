...
class PrintProductLabel(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if 'label_type_id' in fields_list and not default_vals.get('label_type_id'):
            default_vals['label_type_id'] = self.env.ref('product_labels.type_product').id
        return default_vals
    ...
    def _compute_is_template_report(self):
        for wizard in self:
            # flake8: noqa: E501
            wizard.is_template_report = self.report_id == self.env.ref('product_labels.action_report_product_label_from_template')
    ...
    def get_quick_report_action(
        ...
        report = self.env.user.print_label_report_id \
                 or self.env.ref('product_labels.action_report_product_label_60x40_discount')
